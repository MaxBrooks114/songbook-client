...
import { millisToMinutesAndSeconds, renderBool, renderText } from '../../helpers/detailHelpers'
...
const SectionDetail = ({ section, nextSection, prevSection, showDetail }) => {
  ...
  const renderRecordings = () => {
    return recordings.map(recording => {
      const name = recording.file.split('/').slice(-2).join('')
      return (
  ...
  const renderTabs = () => {
    return tabs.map(tab => {
      const name = tab.file.split('/').slice(-2).join('')
      return (
                  ...
                  <Grid container justify="flex-start" alignItems="center">
                    ...
                    <Grid item xs={12} md={12} lg={7}>
                      ...
                      <Typography variant={matches ? 'subtitle1' : 'h6'}>
                        <Link className={classes.link} to={`/songs/${section.song.id}`}>{section.song.title}</Link>
                      </Typography>
                    </Grid>
                  </Grid>
